...
def download(driver,divData,ty):
	j=0
	for i in range(len(divData)):
		...
		if ty.lower() in title.lower():
			btn=driver.find_element_by_xpath('//*[@id="showoutput"]/table/tbody/tr['+str(i+1)+']/td[4]/a')
			btn.click()
			j=j+1
	time.sleep(3)
	try:
		os.rename(dpath+'/display_causelist.pdf',path+'/Tripura/'+ty+' List/'+name+'/list_1.pdf')
	except:
		pass
	for i in range(1,j):
		try:
			os.rename(dpath+'/display_causelist ('+str(i)+').pdf',path+'/Tripura/'+ty+' List/'+name+'/list_'+str(i+1)+'.pdf')
		except:
			pass
def extract():
	...
	download(driver,divData,'Daily')
	download(driver,divData,'Supplementary')
