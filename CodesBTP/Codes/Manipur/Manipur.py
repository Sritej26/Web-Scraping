...
def download(driver,divData,ty):
	...
	time.sleep(3)
	try:
		os.rename(dpath+'/display_causelist.pdf',path+'/Manipur/'+ty+' List/'+name+'/list_1.pdf')
	except:
		pass
	for i in range(1,j):
		try:
			os.rename(dpath+'/display_causelist ('+str(i)+').pdf',path+'/Manipur/'+ty+' List/'+name+'/list_'+str(i+1)+'.pdf')
		except:
			pass
def extract():
