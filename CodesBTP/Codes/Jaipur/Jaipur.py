...
def rename(date,ty):
	for fname in os.listdir(dpath):
		if "SearchCauselist" in fname:
			os.rename(dpath+'/'+fname,path+'/Jaipur/'+ty+'/'+name+'.html')
			break
...
def main():
	url="https://hcraj.nic.in/cishcraj-jp/causelists/"
	#chrome_options = webdriver.ChromeOptions()
	#chrome_options.add_argument("--headless")
	driver=webdriver.Chrome(executable_path=cpath)#,chrome_options=chrome_options)
	driver.get(url)
	driver.command_executor._commands["send_command"] = ("POST", '/session/$sessionId/chromium/send_command')
	#params = {'cmd': 'Page.setDownloadBehavior', 'params': {'behavior': 'allow', 'downloadPath': path+"/Jaipur"}}
